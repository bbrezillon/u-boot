 ...
 */
U_BOOT_CMD_WITH_SUBCMDS(env, "environment handling commands", env_help_text,
#if defined(CONFIG_CMD_ASKENV)
	U_BOOT_SUBCMD_MKENT(ask, CONFIG_SYS_MAXARGS, 1, do_env_ask),
#endif
	U_BOOT_SUBCMD_MKENT(default, 1, 0, do_env_default),
	U_BOOT_SUBCMD_MKENT(delete, CONFIG_SYS_MAXARGS, 0, do_env_delete),
#if defined(CONFIG_CMD_EDITENV)
	U_BOOT_SUBCMD_MKENT_COMPLETE(edit, 2, 0, do_env_edit, var_complete),
#endif
#if defined(CONFIG_CMD_ENV_CALLBACK)
	U_BOOT_SUBCMD_MKENT(callbacks, 1, 0, do_env_callback),
#endif
#if defined(CONFIG_CMD_ENV_FLAGS)
	U_BOOT_SUBCMD_MKENT(flags, 1, 0, do_env_flags),
#endif
#if defined(CONFIG_CMD_EXPORTENV)
	U_BOOT_SUBCMD_MKENT(export, 4, 0, do_env_export),
#endif
#if defined(CONFIG_CMD_GREPENV)
	U_BOOT_SUBCMD_MKENT_COMPLETE(grep, CONFIG_SYS_MAXARGS, 1,
				     do_env_grep, var_complete),
#endif
#if defined(CONFIG_CMD_IMPORTENV)
	U_BOOT_SUBCMD_MKENT(import, CONFIG_SYS_MAXARGS, 0, do_env_import),
#endif
	U_BOOT_SUBCMD_MKENT_COMPLETE(print, CONFIG_SYS_MAXARGS, 1,
				     do_env_print, var_complete),
#if defined(CONFIG_CMD_RUN)
	U_BOOT_SUBCMD_MKENT_COMPLETE(run, CONFIG_SYS_MAXARGS, 1, do_run,
				     var_complete),
#endif
#if defined(CONFIG_CMD_SAVEENV) && !defined(CONFIG_ENV_IS_NOWHERE)
	U_BOOT_SUBCMD_MKENT(save, 1, 0, do_env_save),
#endif
#if defined(CONFIG_CMD_ENV_EXISTS)
